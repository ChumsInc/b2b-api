...
async function loadColors({id = null, code = null} = {}) {
    try {
        const query = `SELECT colors_id AS id, color_code AS code, color_name AS name
                       FROM b2b_oscommerce.colors
                       WHERE (colors_id = :id OR isnull(:id))
                       OR (color_code = :code OR isnull(:code))
                       ORDER BY code`;
        const data = {id};
...
async function saveColor({id, code, name}) {
    try {
        ...
        const connection = await mysql2Pool.getConnection();
        if (!id || id === "0") {
            const [{insertId}] = await connection.query(queryInsert, data);
            id = insertId;
        } else {
            await connection.query(queryUpdate, data);
        }
        connection.release();
