...
const debug = require('debug')('chums:lib:product:mix');
const {mysql2Pool} = require('chums-local-modules');
const {send} = require('../base');
 ...
 */
async function loadItems({mixID, productsID}) {
    try {
 ...
 * @param {number?} [params.productsID]
 */
async function load({mixID, productsID}) {
    try {
...
async function getProductMix(req, res, next) {
    try {
        let products = [];
        ...
        if (products.length === 0 && req.params.id) {
            products = [{id: req.params.id, sellAs: productUtils.SELL_AS.MIX}];
        }
        for await (const prod of products) {
            if (productUtils.checkSellAs(productUtils.SELL_AS.MIX, prod.sellAs)) {
                const [mix] = await load({productsID: prod.id});
                prod.mix = mix || null;
            }
        }
...
async function get(req, res, next) {
    try {
        const [mix] = await load({productsID: res.params.productId});
...
router.get('/:productId(\\d+)', [get, send]);
router.get('/sage/:productId(\\d+)', [getSageBillDetail, send]);
