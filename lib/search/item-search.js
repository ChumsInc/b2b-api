    ...
    try {
        ...
        const sql = `SELECT DISTINCT i.Company,
                     ...
                     WHERE i.Company = 'chums'
                       ...
                       AND i.ItemCode NOT REGEXP
                           '(AAF|ACD|AMZ|AZ|BIG|BPR|BT|BUC|DV|EC|FBA|FIN|PRO|REI|SC|SCH|SPO|TYF|UL|WM|WM49)$'
                       AND i.ItemCode NOT LIKE 'PL%'
                     ORDER BY i.ItemCode
                     LIMIT 50`;
        ...
        if (err instanceof Error) {
            debug("loadSearch()", err.message);
            return Promise.reject(err);
        }
        debug("loadSearch()", err);
...
export const getItemSearch = async (req, res) => {
    try {
        const items = await loadSearch(req.query.search);
        res.json({items});
    } catch(err) {
        if (err instanceof Error) {
