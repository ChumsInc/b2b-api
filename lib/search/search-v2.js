...
const debug = Debug('chums:lib:search:v2');
 ...
 * @returns {Promise.<*>}
 */
async function loadProduct({term}) {
    try {
...
async function loadProducts({term, limit}) {
    const fullterm = `[[:<:]]${term}[[:>:]]`;
    const partterm = `[[:<:]]${term}`;
...
export function getSearchPages(req, res, next) {
...
export function getSearchProduct(req, res, next) {
    const term = prepTerm(req.params.term);
    const site = res.locals.site || req.app.locals.site;
...
export function getSearchProducts(req, res, next) {
    const term = prepTerm(req.params.term);
    const site = res.locals.site || req.app.locals.site;
...
export function sendSearchResult(req, res) {
    const term = prepTerm(req.params.term);
    const site = res.locals.site || req.app.locals.site;
