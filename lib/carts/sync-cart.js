...
export async function syncFromC2({ cartId, customerKey, }) {
    try {
        ...
        let [res] = await mysql2Pool.query(sqlUpdateStatus, { cartId, customerKey: b2bCartCustomerKey });
        updates.closed = res.affectedRows;
        [res] = await mysql2Pool.query(sqlHeader, { cartId, customerKey: b2bCartCustomerKey });
        updates.header = res.affectedRows;
        await mysql2Pool.query(sqlDetailPrep, { cartId, customerKey: b2bCartCustomerKey });
        [res] = await mysql2Pool.query(sqlDetail, { cartId, customerKey: b2bCartCustomerKey });
        updates.detail = res.affectedRows;
        [res] = await mysql2Pool.query(sqlDetailClean, { cartId, customerKey: b2bCartCustomerKey });
...
export async function postSyncCarts(req, res) {
    try {
        ...
        const cartId = req.query.id ?? undefined;
