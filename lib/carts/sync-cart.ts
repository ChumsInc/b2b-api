...
import {B2BCartSyncHeader, B2BCartSyncLine, SageSalesOrderResponse} from "./types.js";
...
export async function syncFromC2({
    ...
    try {
        ...
        let [res] = await mysql2Pool.query<ResultSetHeader>(sqlHeader, {id, customerKey});
        updates.header = res.affectedRows;

        await mysql2Pool.query<ResultSetHeader>(sqlDetailPrep, {id, customerKey: b2bCartCustomerKey});
        [res] = await mysql2Pool.query<ResultSetHeader>(sqlDetail, {id, customerKey});
        updates.detail = res.affectedRows;

        [res] = await mysql2Pool.query<ResultSetHeader>(sqlDetailClean, {id, customerKey: b2bCartCustomerKey});
